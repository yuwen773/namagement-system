  ...
  <div class="performance-review-container">
    ...
    <el-card class="data-card" shadow="hover">
      ...
      >
        ...
        <el-table-column prop="employee_name" label="被评估员工" min-width="120" />
        ...
        <el-table-column prop="score" label="评分" width="100" align="center">
          <template #default="{ row }">
            <el-tag v-if="row.score" :type="getScoreType(row.score)" size="small">
              {{ row.score }}分
            </el-tag>
    ...
    >
      ...
      >
        ...
        <el-form-item label="评分" prop="score">
          <el-rate v-model="formData.score" :colors="['#99A9BF', '#F7BA2A', '#FF9900']" />
    ...
    <el-dialog v-model="detailDialogVisible" title="绩效评估详情" width="600px">
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="评分">
          <el-tag v-if="currentReview.score" :type="getScoreType(currentReview.score)">
        ...
        <el-descriptions-item label="改进建议">
          <div class="content-text">{{ currentReview.improvements || '-' }}</div>
        </el-descriptions-item>
...
// 表单验证规则
const formRules = {
  employee: [{ required: true, message: '请选择被评估员工', trigger: 'change' }],
  review_period: [{ required: true, message: '请选择评估周期', trigger: 'change' }]
}
...
const handlePublish = async (id) => {
  try {
    await ElMessageBox.confirm('确定要发布此绩效评估吗？发布后员工将可以看到。', '确认发布', {
...
const handleUnpublish = async (id) => {
  try {
    await ElMessageBox.confirm('确定要撤回此绩效评估吗？撤回后员工将无法看到。', '确认撤回', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
...
const handleDelete = async (id) => {
  try {
    await ElMessageBox.confirm('确定要删除此绩效评估吗？此操作不可恢复。', '确认删除', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'danger'
    })
